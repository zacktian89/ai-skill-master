...
fn discover_plugins(skills: &[Skill]) -> Vec<crate::models::Plugin> {
            ...
            if plugin_json_path.exists() {
                if let Ok(content) = fs::read_to_string(&plugin_json_path) {
                    if let Ok(val) = serde_json::from_str::<serde_json::Value>(&content) {
                        ...
                        let skills_rel = val["skills"].as_str().unwrap_or("./skills/");
                                ...
                                for entry in entries.flatten() {
                                    if let Ok(file_type) = entry.file_type() {
                                        if file_type.is_dir() {
                                            ...
                                            if let Some(s) = skills.iter().find(|s| s.id == skill_name || s.name == skill_name) {
                                                plugin_skills.push(s.clone());
                                            }
                                        }
                                    }
                                }
                        ...
                        plugins.push(crate::models::Plugin {
                            id: format!("codex-{}", id),
                            name,
                            description,
                            path: dir,
    ...
    // 3. Fallback (若无插件，提供高保真内置插件)
    if plugins.is_empty() {
        let superpowers_path = home.join(".codex").join("plugins").join("cache").join("openai-curated").join("superpowers").join("5e86d584");
        let superpowers_skills: Vec<Skill> = skills
            .iter()
            .filter(|s| s.id == "brainstorming" || s.id == "android-cli" || s.id == "hatch-pet")
            .cloned()
            .collect();
        ...
        mcp_config.insert("command".to_string(), serde_json::Value::String("node".to_string()));
        let mut args = Vec::new();
        args.push(serde_json::Value::String("C:\\Users\\TINNOVE\\AppData\\Roaming\\npm\\node_modules\\@modelcontextprotocol\\server-filesystem\\dist\\index.js".to_string()));
        args.push(serde_json::Value::String("C:\\Users\\TINNOVE\\.gemini\\antigravity-ide".to_string()));
        mcp_config.insert("args".to_string(), serde_json::Value::Array(args));
...
fn built_in_target_profiles() -> Vec<SkillTargetProfile> {
    ...
    vec![
        ...
        SkillTargetProfile {
            id: "antigravity-user".to_string(),
            target_name: "Antigravity".to_string(),
            root_path: home.join(".gemini").join("antigravity").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "amp-user".to_string(),
            target_name: "Amp".to_string(),
            root_path: home.join(".config").join("agents").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "kilocode-user".to_string(),
            target_name: "Kilo Code".to_string(),
            root_path: home.join(".kilocode").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "roo-user".to_string(),
            target_name: "Roo Code".to_string(),
            root_path: home.join(".roo").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "goose-user".to_string(),
            target_name: "Goose".to_string(),
            root_path: home.join(".config").join("goose").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "openclaw-user".to_string(),
            target_name: "OpenClaw".to_string(),
            root_path: home.join(".openclaw").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "droid-user".to_string(),
            target_name: "Droid".to_string(),
            root_path: home.join(".factory").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "trae-user".to_string(),
            target_name: "TRAE IDE".to_string(),
            root_path: home.join(".trae").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "cline-user".to_string(),
            target_name: "Cline".to_string(),
            root_path: home.join(".agents").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "deepagents-user".to_string(),
            target_name: "Deep Agents".to_string(),
            root_path: home.join(".deepagents").join("agent").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "firebender-user".to_string(),
            target_name: "Firebender".to_string(),
            root_path: home.join(".firebender").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "kimi-user".to_string(),
            target_name: "Kimi Code CLI".to_string(),
            root_path: home.join(".config").join("agents").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "replit-user".to_string(),
            target_name: "Replit".to_string(),
            root_path: home.join(".config").join("agents").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "warp-user".to_string(),
            target_name: "Warp".to_string(),
            root_path: home.join(".agents").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "augment-user".to_string(),
            target_name: "Augment".to_string(),
            root_path: home.join(".augment").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "bob-user".to_string(),
            target_name: "IBM Bob".to_string(),
            root_path: home.join(".bob").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "command_code-user".to_string(),
            target_name: "Command Code".to_string(),
            root_path: home.join(".commandcode").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "continue-user".to_string(),
            target_name: "Continue".to_string(),
            root_path: home.join(".continue").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "cortex-user".to_string(),
            target_name: "Cortex Code".to_string(),
            root_path: home.join(".snowflake").join("cortex").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "crush-user".to_string(),
            target_name: "Crush".to_string(),
            root_path: home.join(".config").join("crush").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "iflow-user".to_string(),
            target_name: "iFlow CLI".to_string(),
            root_path: home.join(".iflow").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "junie-user".to_string(),
            target_name: "Junie".to_string(),
            root_path: home.join(".junie").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "kode-user".to_string(),
            target_name: "Kode".to_string(),
            root_path: home.join(".kode").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "mcpjam-user".to_string(),
            target_name: "MCPJam".to_string(),
            root_path: home.join(".mcpjam").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "mistral_vibe-user".to_string(),
            target_name: "Mistral Vibe".to_string(),
            root_path: home.join(".vibe").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "mux-user".to_string(),
            target_name: "Mux".to_string(),
            root_path: home.join(".mux").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "neovate-user".to_string(),
            target_name: "Neovate".to_string(),
            root_path: home.join(".neovate").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "openhands-user".to_string(),
            target_name: "OpenHands".to_string(),
            root_path: home.join(".openhands").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "pi-user".to_string(),
            target_name: "Pi".to_string(),
            root_path: home.join(".pi").join("agent").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "pochi-user".to_string(),
            target_name: "Pochi".to_string(),
            root_path: home.join(".pochi").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "qoder-user".to_string(),
            target_name: "Qoder".to_string(),
            root_path: home.join(".qoder").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "qwen-user".to_string(),
            target_name: "Qwen Code".to_string(),
            root_path: home.join(".qwen").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "trae_cn-user".to_string(),
            target_name: "TRAE CN".to_string(),
            root_path: home.join(".trae-cn").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "zencoder-user".to_string(),
            target_name: "Zencoder".to_string(),
            root_path: home.join(".zencoder").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "adal-user".to_string(),
            target_name: "AdaL".to_string(),
            root_path: home.join(".adal").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "hermes-user".to_string(),
            target_name: "Hermes Agent".to_string(),
            root_path: home.join(".hermes").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "qclaw-user".to_string(),
            target_name: "QClaw".to_string(),
            root_path: home.join(".qclaw").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "easyclaw-user".to_string(),
            target_name: "EasyClaw".to_string(),
            root_path: home.join(".easyclaw").join("skills"),
            scope: ReferenceScope::User,
        },
        SkillTargetProfile {
            id: "autoclaw-user".to_string(),
            target_name: "AutoClaw".to_string(),
            root_path: home.join(".openclaw-autoclaw").join("skills"),
            scope: ReferenceScope::User,
        },
    ]
}
